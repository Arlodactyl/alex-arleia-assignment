...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // rAF token for smoothing font size updates
  let fontSizeRaf = null;
  ...
  function initializeSettingsControls() {
    ...
    if (savedFontSize !== null && fontSizeSlider) {
      const fontSize = parseInt(savedFontSize, 10) || 100;
    ...
    if (fontSizeSlider) {
      ...
      // persist once when user finishes dragging
      fontSizeSlider.addEventListener('change', (e) => {
        const size = parseInt(e.target.value, 10);
        localStorage.setItem(SETTINGS_KEYS.fontSize, size.toString());
        playSettingsSound && playSettingsSound();
      });
    }
    if (soundToggle) {
  ...
  // (smooth with requestAnimationFrame, persist on 'change')
  // ------------------------------------------------------------
  function handleFontSize(event) {
    ...
    // throttle actual apply work
    if (fontSizeRaf) cancelAnimationFrame(fontSizeRaf);
  ...
  // ------------------------------------------------------------
  // APPLY SETTINGS ON ALL PAGES - reads saved prefs and applies immediately
  // ------------------------------------------------------------
  function applyAllSettings() {
    // apply saved font size
    const savedFontSize = parseInt(localStorage.getItem(SETTINGS_KEYS.fontSize) || '100', 10);
    applyFontSize(savedFontSize);

    // apply saved theme
    const savedInvert = localStorage.getItem(SETTINGS_KEYS.invertColours);
    if (savedInvert !== null) {
      applyInvertColours(savedInvert === 'true');
    }

    // ensure sound key exists so other pages can read it
    if (localStorage.getItem(SETTINGS_KEYS.sound) === null) {
      localStorage.setItem(SETTINGS_KEYS.sound, 'true');
    }
  }
});
